...
1. Structure / lane purity / canon single-source:
   `apps/api/.venv/Scripts/python.exe .claude/tools/contract_lint.py $ARGUMENTS`
2. Runtime interface conformance — only if the backend is running AND the feature's
   ...
   `apps/api/.venv/Scripts/python.exe .claude/tools/interface_conformance.py --contract .claude/contracts/$ARGUMENTS/INTERFACE_CONTRACT.md --url http://127.0.0.1:8000`
